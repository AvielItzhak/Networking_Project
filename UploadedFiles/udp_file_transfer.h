...
/* this */ 
int ServerResponseHandleACK(int sockfd, struct sockaddr_in server_addr,  socklen_t server_addr_len, int16_t Seq_NUM, char *OPNAME) {
       
        int Server_Response = 0; // initilazied Loop condition
        unsigned char ACK_Resp_buf[4] = {0}; 

        // Reciving bytes from server
        Server_Response = recvfrom(sockfd, ACK_Resp_buf, MAX_BUFFER_SIZE, 0,
        ...
        // Incase for incoming bytes print Response
        else if ( strcasecmp(OPNAME,"delete") == 0 ) // DELETE
        {    
            printf("\nServer Response: '%s'\n",  ACK_Resp_buf);
        ...
        else if ( strcasecmp(OPNAME,"upload") == 0 ) // UPLOAD
